...
top_counties <- county_shp %>%
  mutate(prop_devcan = devcan/tdevelop) %>%
  arrange(desc(pop2000)) %>%
  head(50)
top_counties %>%
  ggplot(aes(x = prop_devcan, y = 1, fill=stat(x))) +
...
inner_join(state_region, by = c("name00" = "state")) %>%
  group_by(region) %>%
  mutate(median_devcan = quantile(prop_devcan, 0.5, na.rm=TRUE))

region_stats %>% select(region, median_devcan)

region_stats %>%
  ggplot(aes(x = prop_devcan, y = reorder(region, -median_devcan), fill = stat(x), colour = prop_devcan)) +
...
states_dbf %>%
  ...
  inner_join(state_region, by = c("name00" = "state")) %>%
  ggplot(aes(x = prop_us_forest, y = prop_forest, colour = region)) +
  geom_point(size = 3) +
...
ga_me <- county_shp %>%
  filter(name00 %in% c("Georgia", "Maine"))

gg_ga <- ga_me %>%
  filter(name00 == "Georgia") %>%
  ggplot() +
  ...
  geom_sf(data = state_pop_stats %>%
    filter(name00 == "Georgia", cumprop_pop <= 0.75), aes(alpha = log(prop_developed)),
    lwd=0, colour = "#828282") +
...
gg_me <- ga_me %>%
  ...
  scale_fill_gradient(low = "#EED5B7", high = "#228B22", labels = scales::percent, breaks = c(0, 0.4, 0.8)) +
  geom_sf(data = state_pop_stats%>%
    filter(name00 == "Maine", cumprop_pop <= 0.75), aes(alpha = log(prop_developed)),
    lwd=0, colour = "#828282") +
  guides(fill=FALSE, alpha=FALSE)
...
county_stats %>%
  ...
  ggplot(aes(x = cumprop_pop, y = cumprop_area, group=name00)) +
...
state_devcan <- state_pop_stats_dbf %>%
  inner_join(states_dbf %>% select(name00, prop_forest), by="name00", suffix=c("", "_state")) %>%
  mutate(prop_devcan = devcan / tdevelop,
         plot_label = glue::glue("{name00} ({scales::percent(prop_forest_state)})"))
state_devcan_rank = state_devcan %>% group_by(name00) %>% summarise(mean_devcan = mean(prop_devcan, na.rm=TRUE))
state_devcan %>%
  inner_join(state_devcan_rank) %>%
  filter(name00 %in% c("Georgia", "Maine", "Connecticut", "Vermont", "Kentucky", "California", "Mississippi", "North Dakota")) %>%
  ggplot(aes(x = prop_devcan)) +
  ggridges::geom_density_ridges(aes(y=reorder(plot_label, prop_forest_state), fill = prop_forest_state), scale = 2) +
  guides(fill=FALSE) +
  scale_fill_gradient(low = "#EED5B7", high = "#228B22", labels = scales::percent, breaks = c(0, 0.4, 0.8)) +
  labs(y="",
       x = "Canopy Coverage",
...
state_pop_stats %>%
  filter(name00 == "California") %>%
  select(county, pop2000, cumsum_pop, cumprop_pop, populated, prop_forest, popden00) %>%
  ggplot() +
  geom_sf(aes(fill = prop_forest), lwd = 0) +
  scale_fill_gradient(low = "#EED5B7", high = "#228B22", labels = scales::percent, breaks = c(0, 0.4, 0.8)) +
  geom_sf(data = state_pop_stats %>%
    filter(name00 == "California", cumprop_pop <= 0.75), lwd=0, alpha = 3 / 5, fill = "#8A8A8A", colour = "#828282") +
  labs(
    title = "California forests are far from population centers",
    subtitle = "Forest coverage compared to population centers (grey)"
  ) +
  geom_sf(data = states_shp %>% filter(name00 %in% c("Oregon", "Nevada", "Arizona", "Idaho", "Utah"))) +
  # thick california border
  geom_sf(data = states_shp %>% filter(name00 == "California"), lwd=1.2, alpha=0) +
  coord_sf(
    crs = st_crs(4267),
    xlim = c(-127, -113), ylim = c(32, 43), expand = FALSE
  ) +
  theme(
